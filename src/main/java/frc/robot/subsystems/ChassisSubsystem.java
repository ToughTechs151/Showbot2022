...
public class ChassisSubsystem extends SubsystemBase {
  ...
  private MotorControllerGroup leftMotors_ =
      new MotorControllerGroup(frontLeftMotor_, rearLeftMotor_);
  private MotorControllerGroup rightMotors_ =
      new MotorControllerGroup(frontRightMotor_, rearRightMotor_);
  ...
  /**
   * The minimum (closest to 0) speed controller command for the right side of the drive train to
   * start moving forward. Must be empirically derived.
   */
  private static double mechDeadbandRightForward = 0.25;

  /**
   * The maximum (closest to 0) speed controller command for the right side of the drive train to
   * start moving backward. Must be empirically derived. Must be negative.
   */
  private static double mechDeadbandRightBackward = -0.27;

  /**
   * The minimum (closest to 0) speed controller command for the left side of the drive train to
   * start moving forward. Must be empirically derived.
   */
  private static double mechDeadbandLeftForward = 0.23;

  /**
   * The maximum (closest to 0) speed controller command for the left side of the drive train to
   * start moving backward. Must be empirically derived. Must be negative.
   */
  private static double mechDeadbandLeftBackward = -0.29;

  /**
   * The minimum joystick value to actually send a command to the speed controller, to prevent noise
   * near 0.
   */
  ...
  /**
   * The Archade drive uses the speed to control to forwrad and backwar movement where the rotation
   * is for making left or right turn
   *
   ...
   * @param scale Whether or not to scale output values to account for mechanical deadband. 0 is no
   *     scaling; 1 is linear scaling; 2 is quadratic scaling
   */
  public void drive(OI oi, int scale) {
    ...
    if (dir == 1) {
      rightVal =
          getScaledValue(
              oi.getJoystick().getRawAxis(Constants.RIGHT_JOYSTICK_Y), scale, RobotSide.RIGHT);
      leftVal =
          getScaledValue(
              oi.getJoystick().getRawAxis(Constants.LEFT_JOYSTICK_Y), scale, RobotSide.LEFT);
    } else if (dir == -1) {
      rightVal =
          getScaledValue(
              oi.getJoystick().getRawAxis(Constants.LEFT_JOYSTICK_Y), scale, RobotSide.RIGHT);
      leftVal =
          getScaledValue(
              oi.getJoystick().getRawAxis(Constants.RIGHT_JOYSTICK_Y), scale, RobotSide.LEFT);
    }
  ...
  private double getScaledValue(double val, int scale, RobotSide side) {
    ...
    if (Math.abs(val) < softwareDeadband) {
      ...
      if (val > 0) {
        return (1 - forward)
                / Math.pow(1 - softwareDeadband, 2)
                * Math.pow(val - softwareDeadband, 2)
            + forward;
      } else {
        return (-1 + back)
                / Math.pow(-1 + softwareDeadband, 2)
   ...
   * @return If the input is outside the range <code> (-softwareDeadband < val < softwareDeadband)
   *     </code>, it is returned. Else, 0 is returned.
   */
